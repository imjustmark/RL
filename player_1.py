...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
        action_state_counter = np.zeros(shape=(ns, na))
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
                # Choose an action from all possible actions and add to the counter of actions per state
                # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
                action = np.random.choice(a=possible_actions)
                action_state_counter[s_current, action] += 1
